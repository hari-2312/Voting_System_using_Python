...
def pollpage(): #page for polling
     ...
     Label(ppage,text='DIGITALISED VOTING!').grid(row=2,column=3)
...
def polls(): #mypolls
    ...
    Label(mpolls,text='Select Poll to vote',font='Helvetica 12 bold').grid(row=1,column=3)
...
def selpl(): #pollresults
    def results():
        ...
        if sel=='-select-':
            ...
            def project():
                ...
                plt.axis('equal')
                plt.show()
                plt.bar(names,votes,align='center',alpha=1.0)
                plt.xticks(names)
                plt.ylabel('VOTES GOTTED')
    ...
    Label(pl,text='Select Pollname for Result',font='Helvetica 12 bold').grid(row=1,column=1)
...
def about():
    messagebox.showinfo('About','Developed by hari')


home=Tk()
home.geometry('400x400')
home.title('Voting Program')
Label(home,text='voting program made in python',font='Helvetica 12 bold',bg='#49B').grid(row=1,column=2)
